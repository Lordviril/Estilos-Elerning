...
- (id)initWithFrame:(CGRect)frame delegate:(id <KLCalendarViewDelegate>)aDelegate
{
  ...
  //  self.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    ...
    [self refreshViewWithPushDirection:nil]; // add tiles to the grid 
		_grid.frame=CGRectMake(11, 90, 712, 365);
		_grid.backgroundColor=[UIColor clearColor];
    return self;
}

- (CGFloat)headerHeight { 
	//return 0.13707f*self.bounds.size.height; 
	return 80;
}
...
- (void)drawDayNamesInContext:(CGContextRef)ctx
{
    ...
    for (NSInteger columnIndex = 0; columnIndex < 7; columnIndex++) {
        ...
        CGFloat fontSize = 20;//0.25f * columnWidth;
        CGFloat xOffset = columnIndex * columnWidth;
        CGFloat yOffset = (0.94f * [self headerHeight]) - fontSize-10;
...
- (void)addUI
{
	...
	UIImageView*imgview=[[UIImageView alloc]initWithFrame:CGRectMake(0, 0, 750, 39)];
	[imgview setAutoresizingMask:UIViewAutoresizingFlexibleWidth];
	imgview.image=[UIImage imageNamed:@"Screen_16_box2.png" ];
    ...
    //[previousMonthButton setImage:[UIImage imageNamed:@"arrow_left_s3A.png"] forState:UIControlStateNormal];
    previousMonthButton.contentHorizontalAlignment = UIControlContentHorizontalAlignmentCenter;
    previousMonthButton.contentVerticalAlignment = UIControlContentVerticalAlignmentCenter;
    [previousMonthButton addTarget:self action:@selector(showPreviousMonth) forControlEvents:UIControlEventTouchUpInside];
    [imgview addSubview:previousMonthButton];
	...
	[imgview addSubview:_selectedMonthLabel];
    ...
    //[nextMonthButton setImage:[UIImage imageNamed:@"arrow_right_s3A.png"] forState:UIControlStateNormal];
    nextMonthButton.contentHorizontalAlignment = UIControlContentHorizontalAlignmentCenter;
    nextMonthButton.contentVerticalAlignment = UIControlContentVerticalAlignmentCenter;
    [nextMonthButton addTarget:self action:@selector(showFollowingMonth) forControlEvents:UIControlEventTouchUpInside];
    [imgview addSubview:nextMonthButton];
