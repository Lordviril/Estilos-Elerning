...
- (void)removeViewAtIndex:(NSInteger)index
{
    ...
    self.itemViews = [[NSMutableDictionary alloc] initWithDictionary:newItemViews];
...
- (CATransform3D)transformForItemView:(UIView *)view withOffset:(CGFloat)offset
{
    ...
    switch (type)
    {
        ...
        {
            CGFloat tilt =  0.65f;//0.9f;
            CGFloat spacing =0.50f;// 0.25f; // should be ~ 1/scrollSpeed;
...
- (UIView *)containView:(UIView *)view
{
	...
	//NSLog(@"width: %f, height: %f", container.frame.size.width, container.frame.size.height);
...
- (void)reloadData
{
	...
	self.itemViews = [[NSMutableDictionary dictionary] retain];
