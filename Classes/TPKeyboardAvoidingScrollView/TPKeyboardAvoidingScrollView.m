...
-(void)adjustOffsetToScrollViewIfNeeded {
    CGFloat visibleSpace = self.bounds.size.height - self.contentInset.top - self.contentInset.bottom;
    CGPoint idealOffset = CGPointMake(0, [self idealOffsetForView:[self findFirstResponderBeneathView:self] withSpace:visibleSpace]);
    [self setContentOffset:idealOffset animated:YES];
}
...
-(void)scrollToActiveView:(UIView *)theView
{
    CGFloat viewMaxY = CGRectGetMaxY(theView.frame)+theView.inputAccessoryView.frame.size.height + theView.superview.frame.origin.y;
	CGFloat availableHeight;
    CGSize viewSize = self.superview.frame.size;
    availableHeight = viewSize.height - 256 - (CGRectGetMinY(self.frame));
...
-(void)scrollToMakeNormalView:(UIView *)theView
{
    CGFloat viewMaxY = CGRectGetMaxY(theView.frame)+theView.inputAccessoryView.frame.size.height + theView.superview.frame.origin.y;
    CGFloat availableHeight;
    CGSize viewSize = self.superview.frame.size;
